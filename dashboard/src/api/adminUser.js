...
export async function deleteFirebaseUser(uid) {
  const res = await fetch(`${API_URL}/user/${uid}`, { method: "DELETE" });
  return res.json();
}
...
export async function blockFirebaseUser(uid) {
  const res = await fetch(`${API_URL}/user/${uid}/block`, { method: "POST" });
  return res.json();
}

export async function unblockFirebaseUser(uid) {
  const res = await fetch(`${API_URL}/user/${uid}/unblock`, { method: "POST" });
  return res.json();
}
